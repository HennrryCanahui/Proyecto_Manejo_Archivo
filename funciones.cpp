...
using namespace std;
string Ubicacion_Carpeta_User, contenido, descifrado, linea, fileType, folderName;
...
// Convierte texto MAYUCULAS A minusculas
string toLower(string text){
    int size = text.size();
    string New_text = text;
    for (int letra =0; letra <size; ++letra){
        New_text[letra] = tolower(text[letra]);
    }
    return New_text;
}
/*
...
vector<string> getFiles(string fileType) {
...
void crear_carpeta(){
    cout << "Introduce el nombre de la carpeta: ";cin >> folderName;
    Ubicacion_Carpeta_User = seleccionarCarpeta();
    filesystem::path newFolderPath = Ubicacion_Carpeta_User + "/" + folderName;
    filesystem::create_directory(newFolderPath);
}
/*
void mover_archivos(string carpeta_seleccionada){

    for (const auto &entry :filesystem::directory_iterator(desktopPath)) {
        if (find(files.begin(), files.end(), entry.path().extension()) != files.end()) {
            filesystem::path newPath = newFolderPath / entry.path().filename();
            filesystem::rename(entry.path(), newPath);
        }
    }
    cout << "¡Archivos movidos con exito!" << endl;

}

*/
/*
 * Funciones para el juego de adivina la palabra
 */
...
string elegirPalabraAlAzar(const vector<string>& palabras) {
    srand(time(0));
    int indiceAleatorio = rand() % palabras.size();
    return palabras[indiceAleatorio];
}

void mostrarPalabraAdivinada(const string& palabraAdivinada) {
    cout << "Palabra adivinada: " << palabraAdivinada << endl;
}

void mostrarIntentosRestantes(int intentos) {
    cout << "Te quedan " << intentos << " intentos." << endl;
}

void jugarAdivinanzaDePalabras() {
    filesystem::path ubicacion_archivo = filesystem::current_path().parent_path() / "palabras.txt";
    string archivo_palabras = ubicacion_archivo.string();
    vector<string> palabras = leerPalabrasDesdeArchivo(archivo_palabras);
    string palabraSecreta = elegirPalabraAlAzar(palabras);
    string palabraAdivinada(palabraSecreta.length(), '_');

    int intentos = palabraSecreta.length();
    int pistas = 3; // Number of available hints
    char letra;
    ...
    while (intentos > 0 && !adivinado) {
        ...
        cout << "Ingresa una letra o selecciona una pista (P): ";
        cin >> letra;

        if (letra == 'P' || letra == 'p') {
            if (pistas > 0) {
                --pistas;
                cout << "Pista: La palabra contiene la letra '" << palabraSecreta[rand() % palabraSecreta.length()] << "'" << endl;
            } else {
                cout << "Ya no tienes pistas disponibles." << endl;
            }
        } else {
...
/*
 * funciones para seber el peso y fecha de modificacion de un archivo
 */
